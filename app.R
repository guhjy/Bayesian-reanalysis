...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      h5("先驗分配參數 (對數尺度)"),
      helpText("修改下方先驗分配的平均值 (Mean) 與標準差 (SD)，或使用預設值。可選擇為樂觀型/悲觀型先驗計算SD以符合特定機率。"),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (input$use_defaults) {
      ...
      # Disable all prior input fields
  ...
  analysis_output <- reactive({
    ...
    # Use isolate() to prevent re-triggering when current_priors changes inside this reactive
    # 自動反應 current_priors 的變化
    ...
    for (prior_key in names(priors_to_use)) {
      ...
      # 2. Calculate posterior results (HPDI, probabilities, original scale median/CI)
       analysis_results <- tryCatch({
           calculate_results(post_mean, post_sd, poi_orig = point_of_interest, is_ratio_or_lnrr = TRUE) # Always TRUE now
       }, error = function(e) {
