...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      h5("先驗分配參數 (對數尺度)"), #
      helpText("修改下方先驗分配的平均值 (Mean) 與標準差 (SD)，或使用預設值。可選擇為樂觀型/悲觀型先驗計算SD以符合特定機率。模糊型先驗固定為 N(0,1)。"), # Modified help text slightly
      checkboxInput("use_defaults", "使用預設先驗參數 (懷疑型/中性型/樂觀型/悲觀型)", value = TRUE), # Modified label slightly
      br(), #
      ...
      # Skeptic Prior Inputs
       ...
       selectInput("plot_prior_select", "選擇用於圖形的先驗:", # Renamed ID, Updated Label
                   choices = c("懷疑型" = "skeptic",
                    ...
                    selected = "neutral"), #
       helpText("此處選擇的先驗將用於繪製『先驗/概似/後驗圖』以及『預測檢查圖』。先驗分配代表分析前對效果量的信念（在對數尺度上定義）。詳細說明請見「背景 & 先驗」分頁。"), # Updated help text
       hr(), #
       tags$h5("參考文獻"), #
       tags$a(href = "https://www.atsjournals.org/doi/suppl/10.1164/rccm.202006-2381CP?role=tab",
        ...
        tags$br(), #
        "An Overview of Theory and Example Reanalysis of the ART Trial", #
    ...
    # --- Main Panel remains the same structure ---
    mainPanel(
       width = 8, #
       tabsetPanel(
         id = "main_tabs", # Added an ID for potential future use
         # ... Tab Panels ...
         tabPanel("分析結果 (Analysis Results)", #
          ...
          # --- UPDATED Help text for results table ---
          helpText(HTML("<b>表格欄位說明:</b><ul>
                        ...
                        <b>結果判讀:</b> 比較不同先驗下的後驗結果。觀察後驗估計是否偏移、區間是否縮小、相關機率是否支持您的研究假設。BF10 > 3 提供支持 H1 的證據，BF10 < 1/3 提供支持 H0 的證據。後驗機率 P(H1|Data) 結合了證據強度 (BF10) 和您對 H1 的先驗信念。<b>ROPE 機率</b> 評估效果是否具有實際意義上的差異。")), # Updated help text including ROPE
         ...
         # --- Bayes Factor Tab remains the same ---
         tabPanel("貝氏因子與分布圖 (Bayes Factor & Plots)",
                  h4("先驗、概似與後驗分佈圖"),
                  helpText("此圖視覺化呈現了您選擇的先驗分配、由數據推導出的概似函數（假設為常態），以及結合兩者得到的後驗分配。所有分佈均繪製在計算尺度（例如對數尺度）上。<b>同時標示了實用等效區間 (ROPE) 的範圍。</b> 請在左側選擇要繪圖的先驗。"), # Added ROPE mention
                  ...
                  p(strong("後驗機率 P(H1|Data):"), "貝氏因子可以結合您對 H1 的先驗信念 (Prior Probability P(H1)，您在側邊欄設定) 來計算 H1 的後驗機率："),
                  ...
                  helpText("請在左側面板調整 Prior 機率 p(H1)，分析結果表格中的後驗機率將會自動更新。")
         ),

         # --- Background Tab remains the same ---
         tabPanel("背景 & 先驗 (Background & Priors)", #
           ...
           p("這些先驗分配是在效果量的 **對數尺度** 上定義的（例如 log(OR)，log(RR)，log(HR)，lnRR）。您可以修改側邊欄中的參數。下方說明基於預設值或計算值。"), #
           ...
           h5("3. 樂觀型 (Optimistic Prior):"), # Removed default description
           uiOutput("desc_optimistic"), #
           p(strong("選擇建議："), "樂觀先驗的平均效果量可以基於先前的研究、臨床意義的閾值，或甚至基於原始隨機對照試驗 (RCT) 計算樣本數時所假設的效果大小來設定."), #
           h5("4. 悲觀型 (Pessimistic Prior):"), # Removed default description
...
server <- function(input, output, session) {
  ...
  # --- ADDED renderText for Vague Prior CI ---
  ...
  analysis_output <- reactive({
    ...
    if (input_method == "single_effect") {
     # --- Validate and process single effect inputs ---
      ...
      m1 <- input$mean1; #
      lci1 <- input$lower_ci1; uci1 <- input$upper_ci1; n1 <- input$n1 #
  ...
  output$results_table <- renderDT({
    ...
    if (length(res) == 0) {
      return(datatable(data.frame(訊息 = "沒有有效的先驗分配可用於分析，或分析失敗。請檢查側邊欄中的先驗參數和輸入數據。"), rownames = FALSE, options = list(searching = FALSE, lengthChange = FALSE, paging = FALSE, info = FALSE))) #
    }
  ...
  # *** NEW: Display Prior/Likelihood/Posterior Plot ***
